...
namespace network
{
  ...
  {
    auto str_port = boost::lexical_cast<std::string>(port);
  ...
  one_ip_from_host(const std::string& host, const std::string port)
  {
    return endpoint_from_host(host, port).address();
  }

  inline boost::asio::ip::address
  one_ip_from_host(const std::string& host, uint16_t port)
  {
  ...
  get_ipv6(const std::string& str)
  {
    ...
    catch (...) // FIXME : Catch specific exceptions
    {
  ...
  inline boost::asio::ip::address_v6
  get_ipv6(const masks::CharT* ch, size_t size)
  {
  ...
  inline std::string
  binary_to_string_ipv6(const masks::CharT* ch, size_t size)
  {
    return get_ipv6(ch, size).to_string();
  }

  inline network::masks::partsize_type
