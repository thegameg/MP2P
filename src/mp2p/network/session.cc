...
namespace network
{
        ...
        id_{id}
  {
    ...
    misc::Logger::cout() << "[" + std::to_string(id_) + "] " + "Opened session (tid=" + s.str() + ")";
  }
      ...
      id_{id}
  {
    ...
    misc::Logger::cout() << "[" + std::to_string(id_) + "] " + "Opening session (" + host + " - "
                             + std::to_string(port) + ")";
    ...
    async_read(s->socket_get(),
                                       ...
                                       std::size_t size_length)
        {
          if (!ec && size_length == sizeof (PACKET_HEADER))
          {
            ...
            misc::Logger::cout() << "[" + std::to_string(s->id_get()) + "] " + "Receiving a message of size: "
          ...
          {
            misc::Logger::cerr() << "[" + std::to_string(s->id_get()) + "] " + "Error while getting size: "
                                  + ec.message();
          }
        }
    );
                       ...
                       dispatcher_type callback)
  {
    async_read(s->socket_get(),
                            ...
                            std::size_t /* length */)
        {
          ...
          {
            misc::Logger::cerr() << "[" + std::to_string(s->id_get()) + "] " + "Error: " + ec.message();
          }
        }
    );
  }
  ...
  void blocking_receive(std::shared_ptr<Session> s, dispatcher_type callback)
  {
    ...
    {
      misc::Logger::cerr() << "[" + std::to_string(s->id_get()) + "] " + "Error while getting size: " + std::string(e.what());
      return;
    }
    ...
    {
      misc::Logger::cerr() << "[" + std::to_string(s->id_get()) + "] " + "Error: " + std::string(e.what());
      return;
    }
    ...
    async_write(s->socket_get(),
                         ...
                         std::size_t /* length */)
        {
          if (!ec)
            ...
            misc::Logger::cerr() << "[" + std::to_string(s->id_get()) + "] " + "Error while sending: " + ec.message();
        }
    );
  }
  ...
  void blocking_send(std::shared_ptr<Session> s, const Packet& packet, dispatcher_type callback)
  {
    ...
    {
      misc::Logger::cerr() << "[" + std::to_string(s->id_get()) + "] " + "Error while sending: " + std::string(e.what());
    }
  }
