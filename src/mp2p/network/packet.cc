...
namespace network
{
  ...
  Packet::Packet(masks::size_type size, masks::fromto_type fromto,
                 masks::what_type what, const masks::CharT* data, copy to_copy)
    : header_{size, {fromto, what}}
  ...
  Packet::Packet(fromto_type fromto, what_type what, std::vector<CharT>&& data)
  ...
  void
  Packet::copy_message(const message_type& message)
  {
    add_message(message.data(), message.size(), copy::Yes);
  }

  const message_type
  Packet::serialize_header() const
  {
  ...
  masks::fromto_type
  fromto_inverse(masks::fromto_type fromto)
  {
    switch (fromto)
    {
